...
--------------------------------------------------`);
	...
	try {
		...
		if (searchAdContractor != null) {
			...
			if (
				adminChannel[0].companyChannelInfo == false ||
				adminChannel == null ||
				searchAdContractor._id == req.decoded._id
			) {
...
--------------------------------------------------`);
	...
	try {
		...
		// receiver caPath 위치
		const receiverCompanyInfo = await dbModels.Company.findOne(
			{ _id: senderContractRes.receiver_company_id },
			{ orgMSPID: 1 }
		);
		console.log(receiverCompanyInfo);
		let receiverCaPath;
