...
exports.getMyEmployeeTest = async (req, res) => {
    console.log(`
--------------------------------------------------
	User : ${req.decoded._id}
	API  : get my company employee
...
--------------------------------------------------`);
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // 회사 등록이 없는 사람은 바로 추가
            const confirmCompany = await dbModels.PendingCompanyRequest.findOneAndUpdate({ member_id: employeeId }, { company_id: companyId, status: "approve" }, { new: true })
            ...
            if (data[i].managerId) {
                ...
                // 직원들의 매니저 변경
                const updateManager = await dbModels.Manager.findOneAndUpdate({ myId: employeeId }, { myManager: mangerId, accepted: true }, { new: true })
