...
--------------------------------------------------
  User : ${req.decoded._id}
  API  : Approved  Request Pending List
  router.put('/approved--request', approvalMngmtCtrl.approvedRequest);
  query: ${JSON.stringify(req.body._id)} pending  request document id
--------------------------------------------------`);
  const dbModels = global.DB_MODELS;
  const data = req.body;
  console.log(data);
