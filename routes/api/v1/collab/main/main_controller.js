...
--------------------------------------------------`);
    ...
    try {
        const spaceHistory = await dbModels.Space.aggregate([
            {
                $match: {
                    members: new mongoose.Types.ObjectId(req.decoded._id),
                },
            },
            {
                $lookup: {
        ...
        const meeting = await dbModels.Meeting.aggregate([
            {
                $match: {
                    enlistedMembers: new mongoose.Types.ObjectId(req.decoded._id),
                },
            },
            {
                $lookup: {
        ...
        const manager = await dbModels.Manager.aggregate([
            {
                $match: {
                    myId: new mongoose.Types.ObjectId(req.decoded._id),
                },
            },
            {
                $lookup: {
