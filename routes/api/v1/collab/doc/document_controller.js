...
--------------------------------------------------`);
	...
	try {
		const criteria = {
			_id: new mongoose.Types.ObjectId(req.query.docId),
		};
...
--------------------------------------------------`);
	...
	try {
		...
		const spaceDocs = await dbModels.Document.aggregate([
			{
				$match: {
					spaceTime_id: new mongoose.Types.ObjectId(spaceId._id),
				},
			},
			{
				$lookup: {
...
--------------------------------------------------`);
	...
	try {
		...
		const spaceDocs = await dbModels.Document.aggregate([
			{
				$match: {
					spaceTime_id: new mongoose.Types.ObjectId(docInfo.spaceTime_id),
				},
			},
			{
				$lookup: {
...
--------------------------------------------------`);
	...
	try {
		...
		const findFileList = await dbModels.UploadFile.aggregate([
			{
				$match: {
					doc_id: new mongoose.Types.ObjectId(req.query.docId),
				},
			},
			{
				$lookup: {
	...
	try {
		...
		if (data.from == "document") {
			getChatInDoc = await dbModels.Chat.aggregate([
				{
					$match: {
						docId: new mongoose.types.ObjectId(data.docId),
					},
				},
				{
					$lookup: {
